...
function buildISOs() {
  ...
  anchor.setDate(anchor.getDate() + (dow === 0 ? -6 : 1 - dow));
...
export default function Planner({ appData, userId, onEditTask }) {
  ...
  // FIX 2: removed weekOffset — window always anchored to today
  ...
  // FIX 1: three-way sidebar classification
  ...
  for (const t of allActive) {
    ...
    if (!days.length) {
      trueUnscheduled.push(t);
    } else if (hasBefore && !hasAfter) {
      scheduledEarlier.push(t);
    } else if (hasAfter) {
      scheduledLater.push(t);
    } else {
      trueUnscheduled.push(t);
    }
  }
  ...
  return (
    <div className="planner">
      {/* FIX 2: nav arrows removed; only action buttons remain */}
      ...
      <div className="planner-layout">
        <div className="planner-sidebar" onDragOver={e => e.preventDefault()} onDrop={onDropSidebar}>
          ...
          {/* FIX 1: scheduled before the visible window */}
          ...
          {/* FIX 1: scheduled after the visible window */}
